...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-animated">
      {/* Glow orbs */}
      <div style={{ position:'fixed', top:'20%', left:'15%', width:400, height:400, background:'radial-gradient(circle, rgba(124,58,237,0.15) 0%, transparent 70%)', pointerEvents:'none' }} />
      <div style={{ position:'fixed', bottom:'20%', right:'15%', width:300, height:300, background:'radial-gradient(circle, rgba(6,182,212,0.1) 0%, transparent 70%)', pointerEvents:'none' }} />
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="gradient-text" style={{ fontSize:28, fontWeight:800, marginBottom:6 }}>CloudVault</h1>
          <p style={{ color:'var(--text-secondary)', fontSize:14 }}>Sign in to your vault</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {error && (
            <div style={{ padding:'12px 16px', background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:8, color:'#f87171', fontSize:14 }}>
              {error}
            </div>
          )}
          <div>
          ...
          <div>
            ...
            <div style={{ position:'relative' }}>
              ...
              <input type={showPw ? 'text' : 'password'} required className="input" style={{ paddingLeft:42, paddingRight:42 }} placeholder="••••••••" value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} />
        ...
        <p style={{ textAlign:'center', marginTop:24, fontSize:14, color:'var(--text-secondary)' }}>
          No account?{' '}
          <Link href="/register" style={{ color:'#a78bfa', textDecoration:'none', fontWeight:500 }}>Create one</Link>
        </p>
      </motion.div>
    </div>
  );
